...
async function getSpaceXLaunchesData() {
  ...
  const launchesJson = await (
    await fetch("https://api.spacexdata.com/v5/launches/upcoming")
  ).json();
  const launchesArr = [];
  for (let i = 0; i < launchesJson.length; i++) {
    const launchJson = launchesJson[i];
    const rocketJson = await (
      await fetch(
        `https://api.spacexdata.com/v4/rockets/${launchJson["rocket"]}`
      )
    ).json();
    const launchPadJson = await (
      await fetch(
        `https://api.spacexdata.com/v4/launchpads/${launchJson["launchpad"]}`
      )
    ).json();
    const newLaunch = {
...
function convertDateToString(date) {
  return `${days[date.getDay()]} ${date.getDate()} ${
    ...
    .toTimeString()
    .split(" ")[0]
    .substring(0, 5)} UTC`; //Tuesday February 12 2013
}
  ...
  .then((launchesArr) => {
    ...
    for (let i = 0; i < launchesArr.length; i += 2) {
      ...
      if (i < launchesArr.length - 1) {
        ...
        section.className = "launch-outer-container";
        section.innerHTML = `${createLaunchInfo(launchObject1)}
    ...
    main.innerHTML += `<h2 class="asteroids-title">Destroy the Asteroids to get to Mars !</h2>`;
    const asteroids = document.createElement("div");
    asteroids.className = "asteroids";
    asteroids.innerHTML = createAsteroids();
    main.append(asteroids);
    setUpAsteroidGame();
  })
  .catch(
    (e) =>
      (document.getElementsByClassName("placeholder")[0].innerHTML = "error")
  );
